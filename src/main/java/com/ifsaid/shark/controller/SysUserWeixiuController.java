...
//    }
    ...
    @RequestMapping(value = "/addOrder",method = RequestMethod.POST)
    public JsonResult addSchoolRepair(@RequestBody TbSysUserWeixiu schoolRepair) {
        System.err.println(schoolRepair.getRepairUserName());
        //System.err.println(schoolRepair.getRepairType());
        int save = tbSysUserWeixiuService.saveOrder(schoolRepair);
        if(save==1) {
            return JsonResult.success();
        } else {
            return JsonResult.fail("添加维修单失败");
        }
    }
